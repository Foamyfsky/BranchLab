...
- `X`: queues, station occupancy, vehicles, onboard cohorts, service state, delays, latent demand regime, and network availability.
- `U`: user intervention.
...
Do not add a generic Bayesian-network library only for appearance. Record typed simulator events and render the dependency chain:
